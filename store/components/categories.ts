export default { 
  state: () => ({
    categories: [
// Men
      {
        id: 'CT_1',
        title: 'Men',
        img: '/images/home-page/categories/men.png',
        header: true,
        banner: true,
        subCatigories: [
          {
            Clothes: [
              {
                title: 'Coats',
                id: 'C_1'
              },
              {
                title: 'Jackets',
                top: true,
                img: 'images/home-page/popular/img-5.png',
                id: 'C_2'
              },
              {
                title: 'Suits',
                id: 'C_3'
              },
              {
                title: 'Dresses',
                id: 'C_4'
              },
              {
                title: 'Cardigans & sweaters',
                id: 'C_5'
              },
              {
                title: 'Sweatshirts & hoodies',
                id: 'C_6'
              },
              {
                title: 'T-shirts',
                top: true,
                img: 'images/home-page/popular/img-2.png',
                id: 'C_7'
              },
              {
                title: 'Pants',
                id: 'C_8'
              },
              {
                title: 'Jeans',
                id: 'C_9'
              },
              {
                title: 'Shorts',
                id: 'C_10'
              },
              {
                title: 'Skirts',
                id: 'C_11'
              },
              {
                title: 'Lingerie & nightwear',
                id: 'C_12'
              },
              {
                title: 'Sportwear',
                id: 'C_13'
              },
              {
                title: 'Swimwear',
                id: 'C_14'
              }
            ]
          },
          {
            Shoes: [
              {
                title: 'Boots',
                id: 'C_15'
              },
              {
                title: 'Flat shoes',
                id: 'C_16'
              },
              {
                title: 'Heels',
                id: 'C_17'
              },
              {
                title: 'Sandals',
                id: 'C_18'
              },
              {
                title: 'Mules',
                id: 'C_19'
              },
              {
                title: 'Sliders',
                id: 'C_20'
              },
              {
                title: 'Sneakers',
                id: 'C_21'
              },
              {
                title: 'Leather',
                id: 'C_22'
              }
            ]
          },
          {
            Accessories: [
              {
                title: 'Bags & bagpacks',
                id: 'C_23'
              },
              {
                title: 'Hats & scarves',
                top: true,
                img: 'images/home-page/popular/img-3.png',
                id: 'C_24'
              },
              {
                title: 'Hair accessories',
                id: 'C_25'
              },
              {
                title: 'Belts',
                id: 'C_26'
              },
              {
                title: 'Jewellery',
                id: 'C_27'
              },
              {
                title: 'Watches',
                id: 'C_28'
              },
              {
                title: 'Sunglasses',
                id: 'C_29'
              },
              {
                title: 'Purses',
                id: 'C_30'
              },
              {
                title: 'Gloves',
                id: 'C_31'
              },
              {
                title: 'Socks & tights',
                id: 'C_32'
              }
            ]
          }
        ]
      },
// Women
      {
        id: 'CT_2',
        title: 'Women',
        img: '/images/home-page/categories/women.png',
        header: true,
        banner: true,
        subCatigories: [
          { 
            Clothes: [
              {
                title: 'Coats',
                top: true,
                img: 'images/home-page/popular/img-6.png',
                id: 'C_33'
              },
              {
                title: 'Jackets',
                id: 'C_34'
              },
              {
                title: 'Suits',
                id: 'C_35'
              },
              {
                title: 'Dresses',
                id: 'C_36'
              },
              {
                title: 'Cardigans & sweaters',
                id: 'C_37'
              },
              {
                title: 'Sweatshirts & hoodies',
                id: 'C_38'
              },
              {
                title: 'Tops',
                top: true,
                img: 'images/home-page/popular/img-1.png',
                id: 'C_39'
              },
              {
                title: 'Pants',
                id: 'C_40'
              },
              {
                title: 'Jeans',
                id: 'C_41'
              },
              {
                title: 'Shorts',
                id: 'C_42'
              },
              {
                title: 'Skirts',
                id: 'C_43'
              },
              {
                title: 'Lingerie & nightwear',
                id: 'C_44'
              },
              {
                title: 'Sportwear',
                id: 'C_45'
              },
              {
                title: 'Swimwear',
                id: 'C_46'
              }
            ]
          },
          {
            Shoes: [
              {
                title: 'Boots',
                id: 'C_47'
              },
              {
                title: 'Flat shoes',
                id: 'C_48'
              },
              {
                title: 'Heels',
                id: 'C_49'
              },
              {
                title: 'Sandals',
                top: true,
                img: 'images/home-page/popular/img-4.png',
                id: 'C_50'
              },
              {
                title: 'Mules',
                id: 'C_51'
              },
              {
                title: 'Sliders',
                id: 'C_52'
              },
              {
                title: 'Sneakers',
                id: 'C_53'
              },
              {
                title: 'Leather',
                id: 'C_54'
              }
            ]
          },
          { 
            Accessories: [
              {
                title: 'Bags & bagpacks',
                id: 'C_55'
              },
              {
                title: 'Hats & scarves',
                id: 'C_56'
              },
              {
                title: 'Hair accessories',
                id: 'C_57'
              },
              {
                title: 'Belts',
                id: 'C_58'
              },
              {
                title: 'Jewellery',
                id: 'C_59'
              },
              {
                title: 'Watches',
                id: 'C_60'
              },
              {
                title: 'Sunglasses',
                id: 'C_61'
              },
              {
                title: 'Purses',
                id: 'C_62'
              },
              {
                title: 'Gloves',
                id: 'C_63'
              },
              {
                title: 'Socks & tights',
                id: 'C_64'
              }
            ]
          }
        ]
      },
// Boys
      {
        id: 'CT_3',
        title: 'Boys',
        header: true,
        banner: false,
        subCatigories: [
          { 
            Clothes: [
              {
                title: 'Coats',
                id: 'C_65'
              },
              {
                title: 'Jackets',
                id: 'C_66'
              },
              {
                title: 'Suits',
                id: 'C_67'
              },
              {
                title: 'Dresses',
                id: 'C_68'
              },
              {
                title: 'Cardigans & sweaters',
                id: 'C_69'
              },
              {
                title: 'Sweatshirts & hoodies',
                id: 'C_70'
              },
              {
                title: 'T-shirts & tops',
                id: 'C_71'
              },
              {
                title: 'Pants',
                id: 'C_72'
              },
              {
                title: 'Jeans',
                id: 'C_73'
              },
              {
                title: 'Shorts',
                id: 'C_74'
              },
              {
                title: 'Skirts',
                id: 'C_75'
              },
              {
                title: 'Lingerie & nightwear',
                id: 'C_76'
              },
              {
                title: 'Sportwear',
                id: 'C_77'
              },
              {
                title: 'Swimwear',
                id: 'C_78'
              }
            ]
          },
          {
            Shoes: [
              {
                title: 'Boots',
                id: 'C_79'
              },
              {
                title: 'Flat shoes',
                id: 'C_80'
              },
              {
                title: 'Heels',
                id: 'C_81'
              },
              {
                title: 'Sandals',
                id: 'C_82'
              },
              {
                title: 'Mules',
                id: 'C_83'
              },
              {
                title: 'Sliders',
                id: 'C_84'
              },
              {
                title: 'Sneakers',
                id: 'C_85'
              },
              {
                title: 'Leather',
                id: 'C_86'
              }
            ]
          },
          { 
            Accessories: [
              {
                title: 'Bags & bagpacks',
                id: 'C_87'
              },
              {
                title: 'Hats & scarves',
                id: 'C_88'
              },
              {
                title: 'Hair accessories',
                id: 'C_89'
              },
              {
                title: 'Belts',
                id: 'C_91'
              },
              {
                title: 'Jewellery',
                id: 'C_92'
              },
              {
                title: 'Watches',
                id: 'C_93'
              },
              {
                title: 'Sunglasses',
                id: 'C_94'
              },
              {
                title: 'Purses',
                id: 'C_95'
              },
              {
                title: 'Gloves',
                id: 'C_96'
              },
              {
                title: 'Socks & tights',
                id: 'C_97'
              }
            ]
          }
        ]
      },
// Girls
      {
        id: 'CT_4',
        title: 'Girls',
        header: true,
        banner: false,
        subCatigories: [
          { 
            Clothes: [
              {
                title: 'Coats',
                id: 'C_98'
              },
              {
                title: 'Jackets',
                id: 'C_99'
              },
              {
                title: 'Suits',
                id: 'C_100'
              },
              {
                title: 'Dresses',
                id: 'C_101'
              },
              {
                title: 'Cardigans & sweaters',
                id: 'C_102'
              },
              {
                title: 'Sweatshirts & hoodies',
                id: 'C_103'
              },
              {
                title: 'T-shirts & tops',
                id: 'C_104'
              },
              {
                title: 'Pants',
                id: 'C_105'
              },
              {
                title: 'Jeans',
                id: 'C_106'
              },
              {
                title: 'Shorts',
                id: 'C_107'
              },
              {
                title: 'Skirts',
                id: 'C_108'
              },
              {
                title: 'Lingerie & nightwear',
                id: 'C_109'
              },
              {
                title: 'Sportwear',
                id: 'C_110'
              },
              {
                title: 'Swimwear',
                id: 'C_111'
              }
            ]
          },
          {
            Shoes: [
              {
                title: 'Boots',
                id: 'C_112'
              },
              {
                title: 'Flat shoes',
                id: 'C_113'
              },
              {
                title: 'Heels',
                id: 'C_114'
              },
              {
                title: 'Sandals',
                id: 'C_115'
              },
              {
                title: 'Mules',
                id: 'C_116'
              },
              {
                title: 'Sliders',
                id: 'C_117'
              },
              {
                title: 'Sneakers',
                id: 'C_118'
              },
              {
                title: 'Leather',
                id: 'C_119'
              }
            ]
          },
          { 
            Accessories: [
              {
                title: 'Bags & bagpacks',
                id: 'C_120'
              },
              {
                title: 'Hats & scarves',
                id: 'C_121'
              },
              {
                title: 'Hair accessories',
                id: 'C_122'
              },
              {
                title: 'Belts',
                id: 'C_123'
              },
              {
                title: 'Jewellery',
                id: 'C_124'
              },
              {
                title: 'Watches',
                id: 'C_125'
              },
              {
                title: 'Sunglasses',
                id: 'C_126'
              },
              {
                title: 'Purses',
                id: 'C_127'
              },
              {
                title: 'Gloves',
                id: 'C_128'
              },
              {
                title: 'Socks & tights',
                id: 'C_129'
              }
            ]
          }
        ]
      },
// Kids
      {
        id: 'CT_5',
        title: 'Kids',
        img: '/images/home-page/categories/kid.png',
        header: false,
        banner: true,
      }
    ]
  }),

  getters: {
    categoriesHeader (state: any, title: any) {
      let categories = state.categories;
      title = 'header';
      let titles = [];
      for(let i=0; i < categories.length; i++) {
        if(categories[i][title] === true) {
          titles.push(categories[i]['id']);
        };
      };
      return titles;
    },
    categoriesBanner (state: any, title: any) {
      let categories = state.categories;
      title = 'banner';
      let titles = [];
      for(let i=0; i < categories.length; i++) {
        if(categories[i][title] === true) {
          titles.push(categories[i]);
        };
      };
      return titles;
    }
  },

  namespaced: true
}
